...
echo "$output" | jq -r --arg args "$common_args" '. as $ips | to_entries[] |
  select(.key == "master") |
  "\(.key) ansible_host=\(.value) ansible_user=ubuntu ansible_ssh_private_key_file=~/.ssh/my_gcp_key \($args)"' >> "$inventory_file"

# Tạo phần [workers]
echo -e "\n[slave]" >> "$inventory_file"
echo "$output" | jq -r --arg args "$common_args" '. as $ips | to_entries[] |
  select(.key | test("^slave")) |
  "\(.key) ansible_host=\(.value) ansible_user=ubuntu ansible_ssh_private_key_file=~/.ssh/my_gcp_key \($args)"' >> "$inventory_file"
